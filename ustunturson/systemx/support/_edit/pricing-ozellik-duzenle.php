<?php echo !defined("GUVENLIK") ? die("Vaoww! Bu ne cesaret?") : null;?>
<title>Pricing Table Özelliği Düzenle | <?=$ayar['site_baslik']?></title>
<?php
$Siralama = $db->prepare("select * from pricing_ozellik where id='$_GET[ozellik_id]' and dil='$_SESSION[dil]' ");
$Siralama->execute();
$row = $Siralama->fetch(PDO::FETCH_ASSOC);

$degerCek = $db->prepare("select * from pricing where id='$row[pr_id]'");
$degerCek->execute();
$pr = $degerCek->fetch(PDO::FETCH_ASSOC);
?>
<?php
if($Siralama->rowCount() == 0 ) {
    header("Location:pages.php?sayfa=pricingler");
}
?>
<div class="row page-titles">
    <div class="col-md-5 align-self-center">
        <h4 class="text-themecolor"><i class="mdi mdi-table-edit"></i> Pricing Table Özelliği Düzenle</h4>
    </div>
    <div class="col-md-7 align-self-center text-right">
        <div class="d-flex justify-content-end align-items-center">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="javascript:void(0)">Yönetim Paneli</a></li>
                <li class="breadcrumb-item"><a href="pages.php?sayfa=pricingler">Pricing Table</a></li>
                <li class="breadcrumb-item active">Pricing Table Özelliği Düzenle</li>
            </ol>
        </div>
    </div>
</div>


<div class="row">
    <div class="col-12">
        <div class="card" >
            <div class="card-body text-white" style="background: linear-gradient(to right, #4568dc, #b06ab3);" >

                <h3 class="card-title" style="margin-bottom: 0 !important;">Tablo :  <?=$pr['baslik']?> </h3>

            </div>

        </div>
    </div>

    <div class="col-lg-12">
        <div class="card">




            <div class="card-body" style="font-family: 'Open Sans', Arial">

                <form action="support/post/update/pricing-ozellik-duzenle.php" class="form-horizontal form-bordered" method="post" enctype="multipart/form-data">

                    <input type="hidden" name="ozellik_id" value="<?=$row['id']?>">
                    <input type="hidden" name="pricing_id" value="<?=$pr['id']?>">

                    <h3 class="card-title">Pricing Table Özelliği Düzenle</h3>
                    <hr>
                    <div class="form-body">




                        <div class="form-group row">
                            <label class="control-label text-right col-md-3" for="ozellikAd" style="margin-top: 9px; margin-bottom: 10px; font-weight: 600">ÖZELLİK YAZISI</label>
                            <div class="col-md-6">
                                <input type="text" name="baslik" required class="form-control" id="ozellikAd" value="<?=$row['baslik']?>">
                            </div>
                        </div>

                        <div class="form-group row">
                            <label class="control-label text-right col-md-3" for="siraArea" style="margin-top: 9px; margin-bottom: 10px; font-weight: 600">Tablo Sırası</label>
                            <div class="col-md-1">
                                <input type="number" name="sira" required class="form-control" id="siraArea" value="<?=$row['sira']?>"  min=1 oninput="validity.valid||(value='');">
                            </div>
                        </div>


                        <div class="form-actions">
                            <button type="submit" class="btn btn-success" name="pricingdegis">
                                <i class="fa fa-refresh fa-spin fa-fw"></i>
                                <span class="sr-only">Yükleniyor...</span> Güncelle
                            </button>
                        </div>



                </form>

                    </div>



            </div>







        </div>
    </div>
</div>


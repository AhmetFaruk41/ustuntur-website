...
if ($adminsorgusu->rowCount()===0) {
    ...
    if ($_GET['hizmet'] == "success") {

        $gorselSil = $db->prepare("select * from hizmet where id='$_GET[id]'");
        $gorselSil->execute();
        while ($row = $gorselSil->fetch(PDO::FETCH_ASSOC)) {
            $gorseladres = $row['gorsel'];

            unlink("../../../../images/services/$gorseladres");
        }

        $sil = $db->prepare("DELETE from hizmet where id=:id");
        $kontrol = $sil->execute(
            array(
                'id' => $_GET['id']
            )

...
if ($adminsorgusu->rowCount()===0) {
    ...
    if (isset($_POST['mobillogodegis'])) {

        if ($_FILES['site_mobil_logo']["size"] > 0) {
            $dosyas = $_FILES["site_mobil_logo"];
            $kaynak = $_FILES["site_mobil_logo"]["tmp_name"];
            $dosya = $_FILES["site_mobil_logo"]["name"];
            $uzanti = explode(".", $_FILES['site_mobil_logo']['name']);
            ...
            if ($dosyas['type'] == 'image/jpg' || $dosyas['type'] == 'image/jpeg' || $dosyas['type'] == 'image/png') {
                ...
                $ayarkaydet = $db->prepare(
                    "UPDATE ayarlar SET
			site_mobil_logo=:site_mobil_logo	
			WHERE id='1'"
                );
                $update = $ayarkaydet->execute(
                    array(
                        'site_mobil_logo' => $yeni_isim
                    )
                );

                if ($update) {

                    $resimsilunlink = $_POST['eski_mobil_logo'];
